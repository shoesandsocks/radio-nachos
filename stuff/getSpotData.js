...
const getWholePlaylist = async (
  ...
  const newArr = accumulator.concat(...chunk.body.items);
  if (chunk.body.total > offset + limit) {
    return getWholePlaylist(identifier, offset + limit, limit, newArr);
  }
  return newArr;
};
...
spotifyApi.clientCredentialsGrant().then(
  async function (data) {
    ...
    // const all = await getWholePlaylist("1EQ6eMB19i1XedKO4kpBW0");
  },
  function (err) {
